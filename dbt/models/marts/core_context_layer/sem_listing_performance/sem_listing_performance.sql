...
-- sem_listing_daily: a cross-sectional comparison of 50 listings, where the
-- daily view would make the same question a 17,885-row aggregation and invite
-- grouping a lifetime measure by date.
--
...
-- question 14 (which amenities go with higher achieved rates) can reach all six
-- flags dim_listings exposes. That comparison is CROSS-SECTIONAL only, and the
...
TABLES (
    listing AS {{ ref('dim_listings') }}
        ...
        COMMENT = 'One row per listing with lifetime measures over the fixed year ending 2022-07-11. All 50 listings the calendar references, including orphan 276450 whose descriptive columns are NULL.',
...
FACTS (
    listing.total_revenue AS listing.total_revenue
        ...
        COMMENT = 'Lifetime booked revenue over the year. Zero, not NULL, for the 3 listings that were never booked - question 5.',

    listing.occupancy_rate AS listing.occupancy_rate
        WITH SYNONYMS = ('occupancy', 'utilization')
        COMMENT = 'Booked nights divided by 365 for this listing. Ranges 0 to 1.',

    listing.avg_nightly_price AS listing.avg_nightly_price
        COMMENT = 'Mean rate this listing was OFFERED at across all 365 nights, booked or not. Not what it earned.',

    listing.list_price AS listing.list_price
        WITH SYNONYMS = ('advertised price', 'asking price')
        COMMENT = 'Advertised nightly rate from the listings table. NULL for the orphan listing.',
    ...
    listing.calendar_days AS listing.calendar_days
        COMMENT = '365 for every listing, since the source is a fixed one-year snapshot. It is the occupancy denominator, NOT a measure of how long the listing has existed.',
    ...
    listing.achieved_nightly_rate
        ...
        COMMENT = 'Revenue divided by nights actually sold - the rate the listing EARNED. NULL for the 3 listings never booked.',
    ...
    listing.days_since_last_review
        ...
        COMMENT = 'Days between the most recent review and the snapshot end of 2022-07-11. Question 25. Measured against as_of_date rather than current_date, so the answer does not drift on every run. NULL for listings never reviewed, which is a different thing from stale.',

    listing.months_since_last_review
        AS datediff(month, listing.last_review_date, listing.as_of_date)
        COMMENT = 'Whole months since the most recent review, anchored to 2022-07-11. Runs up to 108 months - 9 years - on the stalest listing.'
)

DIMENSIONS (
    listing.listing_id AS listing.listing_id
        COMMENT = 'Listing identifier. 50 distinct listings.',

    listing.listing_name AS listing.listing_name
        WITH SYNONYMS = ('name', 'title')
        COMMENT = 'Listing title. NULL for the orphan listing.',

    listing.neighborhood AS listing.neighborhood
        WITH SYNONYMS = ('area', 'district', 'location')
        COMMENT = 'Listing neighborhood. NULL for the orphan listing. Some neighborhoods hold a single listing - Back Bay is one - so treat per-neighborhood averages as thin.',
    ...
    listing.room_type AS listing.room_type
        ...
        COMMENT = 'Entire home/apt, private room, or shared room. 31 entire homes and 18 private rooms.',
    ...
    listing.bedrooms AS listing.bedrooms
        COMMENT = 'Bedroom count. NULL on 8 of 49 listings, all entire homes.',

    listing.beds AS listing.beds
        COMMENT = 'Bed count. Zero on 4 listings, all entire homes. One listing records 1 bedroom and 0 beds, so bedrooms and beds are not substitutes for each other.',
    ...
    listing.is_shared_bathroom AS listing.is_shared_bathroom
        ...
        COMMENT = 'True where the raw bathroom label mentions shared - 9 listings against 39 private. Question 13, which quantifies a renovation case.',
    ...
    listing.as_of_date AS listing.as_of_date
        COMMENT = 'The snapshot end, 2022-07-11, constant on every row. The anchor every recency measure here uses instead of current_date.',

    listing.was_never_booked AS listing.total_revenue = 0
        COMMENT = 'True for the 3 listings available all 365 days that never earned anything - listings 1454258, 1510876 and 743759. Question 5: price, photos, or location.',
    ...
    listing.is_orphan_listing AS listing.is_orphan_listing
        COMMENT = 'True for listing 276450, which appears in 367 calendar rows but has no listings row, so every descriptive column is NULL. Filter it out when comparing attributes - it would otherwise form a NULL group. Leave it in when totalling revenue.',

    -- All six flags dim_listings exposes, for question 14. The daily fact
    -- carries only three; int_listing_amenities has all 81 if a question needs
    -- an amenity with no column here.
    ...
    listing.amenity_count AS listing.amenity_count
        COMMENT = 'Number of amenities on the listing. 81 distinct amenities exist across the portfolio; only these six have their own flag here.',

    host.host_id AS host.host_id
        COMMENT = 'Host identifier. The join key - never use the masked name, which two hosts share.',

    host.host_name_masked AS host.host_name_masked
        COMMENT = 'Salted hash of the host name. The plaintext name is PII and does not exist anywhere in these models. Usable as a grouping key but NOT as a host identifier: two of the 36 hosts share a name and therefore share a hash. Join on host_id.',
    ...
    host.host_tenure_years AS host.host_tenure_years
        COMMENT = 'Whole years between the host joining and 2022-07-11. Little variance - host_since clusters in 2008 to 2009.',

    host.is_multi_listing_host AS host.is_multi_listing_host
        COMMENT = 'True where the host holds more than one listing - 7 hosts holding 20 listings. NULL for the orphan listing, which has no host.',

    host.verification_count AS host.verification_count
        COMMENT = 'Number of verification methods the host completed, 2 to 9.'
)
...
METRICS (
    ...
    -- Question 7 (revenue concentration, share from the top 5) has NO metric
    ...
    -- identical on every room type, because calendar_days is 365 for all 50
    -- listings, so the night-weighted denominator is a constant multiple of the
    -- listing count. They diverge in sem_host_performance, where portfolio sizes
    -- differ — 55.9 percent host-weighted against 54.2 percent night-weighted.
    -- Kept as a pair anyway, so a query written against either view means the
    -- same thing by name, and so the equality here stops holding visibly if the
    -- snapshot ever covers listings for unequal windows.
    listing.avg_occupancy_rate AS avg(listing.occupancy_rate)
        WITH SYNONYMS = ('occupancy', 'average occupancy')
        COMMENT = 'Mean of the per-listing occupancy rate. LISTING-weighted: every listing counts equally. Use this to compare neighborhoods or room types, which is question 4. Equal to occupancy_rate_weighted on this data, since every listing has exactly 365 calendar nights.',

    listing.occupancy_rate_weighted
        AS div0(sum(listing.booked_nights), sum(listing.calendar_days))
        COMMENT = 'Total booked nights divided by total calendar nights. NIGHT-weighted. Identical to avg_occupancy_rate on this data because all 50 listings have 365 calendar nights - it would only diverge if listings covered unequal windows. The two DO differ at host grain, where portfolio sizes vary; see SEM_HOST_PERFORMANCE.',
    ...
    listing.avg_list_price AS avg(listing.list_price)
        ...
        COMMENT = 'Mean advertised rate. Excludes the orphan listing, whose list_price is NULL.',
    ...
    listing.never_booked_listings AS count_if(listing.total_revenue = 0)
        COMMENT = 'How many listings earned nothing all year. 3 overall - question 5.',
    ...
    listing.avg_days_since_last_review AS avg(listing.days_since_last_review)
        COMMENT = 'Mean days since the last review, anchored to 2022-07-11 rather than current_date. Question 25.',

    listing.stalest_days_since_review AS max(listing.days_since_last_review)
        COMMENT = 'Longest gap since a review in the slice, anchored to 2022-07-11.',

    listing.hosts AS count(distinct listing.host_id)
        COMMENT = 'Distinct hosts in the slice. Excludes the orphan listing, whose host_id is NULL.'
)

COMMENT = 'Lifetime performance per rental listing over the fixed year ending 2022-07-11: revenue, occupancy, pricing against the advertised rate, reviews, and attributes including amenities. Use this to compare listings or segments cross-sectionally - which neighborhood or room type performs, which listings never earned, whether reviews or amenities go with higher rates. For anything by date or month use SEM_LISTING_DAILY; for booking counts and length of stay use SEM_RESERVATIONS; for host portfolios use SEM_HOST_PERFORMANCE. IMPORTANT on amenities: any link between an amenity and revenue here is CORRELATION AT A POINT IN TIME, not impact. Every amenity changelog event predates the calendar window - the latest is 2021-07-06 and the window opens 2021-07-12 - so there is no before-and-after period in the data and the revenue effect of ADDING an amenity cannot be measured at any modelling effort.'

AI_SQL_GENERATION 'Measures here are already lifetime totals per listing, so never multiply them by a night count or group them by a date. For recency and staleness use days_since_last_review or months_since_last_review, which anchor to as_of_date - NEVER use current_date, because the snapshot ends 2022-07-11 and current_date makes the answer drift on every run. Distinguish a NULL last_review_date, meaning never reviewed, from a large staleness value, meaning reviewed long ago. When comparing descriptive attributes filter is_orphan_listing to false, since listing 276450 has NULL attributes and would form a NULL group; when totalling revenue leave it in. For occupancy pick deliberately: avg_occupancy_rate is listing-weighted and right for comparing segments, occupancy_rate_weighted is night-weighted and right for a portfolio total. Never present an amenity-to-revenue relationship as causal.'
