...
-- multiplies each listing by its 365 calendar rows. Exposing one grain of
...
TABLES (
    daily AS {{ ref('fct_listing_daily') }}
        ...
        COMMENT = 'One row per listing per date across a fixed one-year snapshot, 2021-07-12 to 2022-07-11. Not a rolling window: the last date is 2022-07-11, so never measure recency against current_date.',
...
RELATIONSHIPS (
    -- Complete: dim_listings is built off the amenities bridge, so all 50
    -- listings the calendar references have a row, including orphan 276450.
    daily_to_listing AS daily (listing_id) REFERENCES listing (listing_id),

    -- NOT complete. The orphan listing has a NULL host_id, so grouping a daily
    -- metric by any host attribute drops its 367 rows and the revenue on them.
...
FACTS (
    ...
    daily.price_per_bedroom AS daily.price / nullif(daily.bedrooms, 0)
        COMMENT = 'Nightly price divided by bedrooms. bedrooms is NULL on 8 of 49 listings, all entire homes, so those rows drop out rather than counting as zero.',

    daily.price_per_bed AS daily.price / nullif(daily.beds, 0)
        COMMENT = 'Nightly price divided by beds. beds is 0 on 4 listings, so nullif is required or the division errors. One listing records 1 bedroom and 0 beds, so bedrooms and beds are not interchangeable.',
...
DIMENSIONS (
    ...
    listing.listing_name AS listing.listing_name
        ...
        COMMENT = 'Listing title, from the listing dimension rather than the fact - a name repeated on 365 rows per listing is the redundancy the star schema exists to avoid. NULL for the orphan listing.',

    daily.calendar_date AS daily.calendar_date
        WITH SYNONYMS = ('date', 'night', 'day')
        COMMENT = 'The date this row describes. Spans 2021-07-12 to 2022-07-11.',
    ...
    daily.reservation_id AS daily.reservation_id
        COMMENT = 'Booking occupying this date, NULL when available. Not unique on its own - id 836 covers two separate stays on different listings. Count reservations in sem_reservations, not here.',
    ...
    daily.availability_window_seq AS daily.availability_window_seq
        ...
        COMMENT = 'Which contiguous run of available nights this date belongs to, numbered per listing. Group by listing_id and this to get one row per availability window. MEANINGFUL ONLY WHERE is_available IS TRUE - always filter is_available before grouping on it. It is a running count of runs started, so a booked night carries the number of the run that closed before it, and a group that omits the filter collects those booked nights and reports a longer window than exists. Window length is the count of nights in the group, never a date difference: every available date is a bookable night, so 2022-02-03 through 2022-07-11 is 159 nights and not 158.',
    ...
    daily.is_orphan_listing AS daily.is_orphan_listing
        COMMENT = 'True for listing 276450, which has 367 calendar rows but no listings row, so its descriptive columns are NULL. Filter it out when comparing attributes. LEAVE IT IN when totalling revenue - it carries real booked revenue, and dropping it changes the answer to question 1 from 21.2 to 22.1 percent.',

    listing.neighborhood AS listing.neighborhood
        WITH SYNONYMS = ('area', 'district', 'location')
        COMMENT = 'Listing neighborhood. NULL for the orphan listing.',
    ...
    listing.beds AS listing.beds
        COMMENT = 'Bed count. Zero on 4 listings, all entire homes.',

    listing.is_shared_bathroom AS listing.is_shared_bathroom
        COMMENT = 'True where the raw bathroom label mentions shared - 9 listings against 39 private. Not reflected in the bathroom count, so this is the only signal for it.',
    ...
    -- pivot all 81 amenities, and the flags it does carry are the ones the
    ...
    host.host_tenure_years AS host.host_tenure_years
        COMMENT = 'Whole years between the host joining and 2022-07-11. Little variance to work with - host_since clusters in 2008 to 2009.',

    host.is_multi_listing_host AS host.is_multi_listing_host
        COMMENT = 'True where the host holds more than one listing - 7 of 36 hosts. Reaching this from the daily grain drops the orphan listing, which has no host.'
)
...
METRICS (
    ...
    daily.calendar_nights AS count(*)
        COMMENT = 'Nights in the window. 365 per listing, so it is the occupancy denominator rather than a measure of listing age.',
    ...
    daily.distinct_prices AS count(distinct daily.price)
        ...
        COMMENT = 'Distinct nightly rates in the slice. Grouped by listing this separates dynamic-pricing hosts from set-and-forget ones - 1 means the rate never moved.',
    ...
    daily.avg_price_per_bedroom AS avg(daily.price_per_bedroom)
        COMMENT = 'Mean of price per bedroom. Backed by 23 of 31 entire homes and all 18 private rooms, since bedrooms is NULL on 8 entire homes. Normalizing compresses the entire-home premium over a private room from 2.4x on raw price to 1.9x.',

    daily.avg_price_per_bed AS avg(daily.price_per_bed)
        COMMENT = 'Mean of price per bed. Backed by 27 of 31 entire homes, since beds is 0 on 4. Compresses the same premium to 1.5x.',
    ...
    -- 7 of the 204 runs - so which end is taken changes the answer.
...
COMMENT = 'Nightly economics for rental listings: revenue, pricing, occupancy and availability at listing x date grain over a fixed year ending 2022-07-11. Use this for anything about a specific date, month, day of week, or price over time. For lifetime per-listing measures use SEM_LISTING_PERFORMANCE, for booking counts and length of stay use SEM_RESERVATIONS, for host portfolios use SEM_HOST_PERFORMANCE.'

AI_SQL_GENERATION 'The calendar is a fixed snapshot ending 2022-07-11. NEVER use current_date or current_timestamp for recency, staleness or tenure - anchor to the max calendar_date or to the as_of_date column instead, or the answer changes on every run. Prefer the precomputed month_start_date over date_trunc on calendar_date. Do not filter out is_orphan_listing when totalling revenue, because listing 276450 carries real booked revenue; do filter it out when comparing descriptive attributes, which are NULL for it. Do not count reservations in this view - reservation_id is not unique here and each booking spans many rows. For anything about a contiguous stretch of open nights, group by listing_id and availability_window_seq with is_available filtered first, and measure the length of a window as the count of rows in the group rather than as a difference between dates.'
