{#
    Verified queries for business question 22 — price per bedroom and per bed.
    ...
    Verified in analyses/06_price_per_bedroom.sql:
      entire home/apt (31)  $216.84 nightly  $165.68/bedroom  $128.10/bed
      private room    (18)  $ 89.11 nightly  $ 85.74/bedroom  $ 84.15/bed

    Normalizing compresses the premium sharply: an entire home looks 2.4x a
    private room on raw price, 1.9x per bedroom and 1.5x per bed. Most of the
    ...
    OTHER. bedrooms is NULL on 8 of 49 listings, so those rows drop out of that
    average. beds is 0 on 4, so nullif is required or the division errors. Both
    gaps land entirely on entire homes - private rooms have complete data - and
    one listing records 1 bedroom against 0 beds. Both guards are inside the
    view's fact definitions, so a query here cannot forget them; what a query CAN
    forget is that the per-bedroom entire-home figure covers 23 listings and not
    31, which is why every entry returns the coverage counts alongside.
    ...
    because every listing has exactly 365 calendar rows. It reproduces
    analyses/06 to the cent for that reason. If the snapshot ever covered listings
    ...
    is_orphan_listing filtered throughout: it has NULL bedrooms, beds and room
    type, so it contributes nothing to either normalization and would form a NULL
    group. Correct here, because the question compares attributes.

    Apostrophes are fine in either field - the dispatcher doubles them for the
    single-quoted SQL literal each is emitted into.
#}
{% macro verified_queries_q22(view=none) -%}
